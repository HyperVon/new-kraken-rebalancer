    ...
    fun DIV.render(latest: PortfolioSnapshot) {
        glassPanel(ViewText.PORTFOLIO_ALLOCATION, Icons.DOLLAR_CIRCLE) {
            div(CssClasses.ALLOCATION_CHART_CONTAINER) {
                ...
                val maxVal =
                    if (topAssets.isNotEmpty()) topAssets.first().valueUSD.toDouble() else 1.0

                topAssets.forEach { asset ->
                    val fillPct =
                        if (maxVal > 0) (asset.valueUSD.toDouble() / maxVal * 100).toInt() else 0
                    div(CssClasses.ALLOCATION_BAR_ROW) {
                        div(CssClasses.ALLOCATION_BAR_LABEL) { +asset.symbol }
