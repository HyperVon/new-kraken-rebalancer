...
class PortfolioManagerEdgeCasesTest : StringSpec() {
    ...
    init {
        ...
        "testPerformRebalanceCycle_TradeHistorySaveIOException" {
            runTest {
                krakenService.balanceSupplier = { mapOf("USD" to 1000.0) }
                val allocs = listOf(Allocation("USD", 100.0))
                val settings = Settings(0L, 2.0, 1.0, true, 0.0, 1.0)
                val config = AppConfig(KrakenCredentials("k", "s"), settings, allocs)
                every { configService.getConfig() } returns config
        ...
        "testResolvePriceFromTicker_FallbackBranches" {
            ...
            val price = portfolioManager.resolvePriceFromTicker("BTC", rawPrices)
            price shouldBe BigDecimal("61000.0")

            val rawPricesOnlyEur = mapOf(
                "XBTEUR" to 55000.0
            )
            val priceEurOnly = portfolioManager.resolvePriceFromTicker("BTC", rawPricesOnlyEur)
            priceEurOnly.compareTo(BigDecimal.ZERO) shouldBe 0
        }

        "testCalculatePortfolioValues_PriceNotFoundAbort" {
            val balances = mapOf("USD" to 1000.0, "BTC" to 1.0)
            val prices = mapOf("USD" to BigDecimal.ONE)
        ...
        "testResolveBalance_FallbackChain" {
            ...
            resolveBalanceMethod.invoke(portfolioManager, "BTC", mapOf("BTC" to 1.1)) shouldBe 1.1
            resolveBalanceMethod.invoke(portfolioManager, "BTC", mapOf("XBTC" to 1.2)) shouldBe 1.2
            resolveBalanceMethod.invoke(portfolioManager, "USD", mapOf("ZUSD" to 1.3)) shouldBe 1.3
            resolveBalanceMethod.invoke(portfolioManager, "BTC", mapOf("XBT" to 1.4)) shouldBe 1.4
            resolveBalanceMethod.invoke(portfolioManager, "BTC", mapOf("XXBT" to 1.5)) shouldBe 1.5
            resolveBalanceMethod.invoke(portfolioManager, "BTC", mapOf("ETH" to 1.6)) shouldBe 0.0
        }
        ...
        "testAnalyzeDeviations_MissingSymbolInCurrentValues" {
            ...
            portfolioManager.analyzeDeviations(totalVal, currentValuesUSD, effUsdTarget, cryptoScale, buyOrders, sellOrders, actionLog)
            buyOrders["BTC"]?.compareTo(BigDecimal("250.0")) shouldBe 0
        }

        "testAnalyzeDeviations_USDTriggerOnlyEnforcesFiatCorrection" {
            val allocs = listOf(Allocation("USD", 20.0), Allocation("BTC", 40.0), Allocation("ETH", 40.0))
            val settings = Settings(0L, 15.0, 1.0, true, 0.0, 1.0)
            every { configService.getConfig() } returns AppConfig(KrakenCredentials("k", "s"), settings, allocs)

            val currentValuesUSD = mapOf("USD" to BigDecimal("240.0"), "BTC" to BigDecimal("380.0"), "ETH" to BigDecimal("380.0"))
            val buyOrders = mutableMapOf<String, BigDecimal>()
            val sellOrders = mutableMapOf<String, BigDecimal>()
            val actionLog = mutableListOf<String>()
            ...
            buyOrders["BTC"]!!.compareTo(BigDecimal("20.0")) shouldBe 0
            buyOrders["ETH"]!!.compareTo(BigDecimal("20.0")) shouldBe 0
        }
        ...
        "testExecuteOrders_DryRunAndSellsSuccess" {
            runTest {
                ...
                val sellOrders = mapOf("BTC" to BigDecimal.valueOf(100.0))
                val currentValuesUSD = mapOf("USD" to BigDecimal.valueOf(1000.0))
                val prices = mapOf("BTC" to BigDecimal.TEN)
                val settings = Settings(0L, 2.0, 1.0, true, 0.0, 1.0)
                val actionLog = mutableListOf<String>()
        ...
        "testRefreshUsdBalanceAfterSells_EarlyReturnAndTimeout" {
            runTest {
                ...
                krakenService.balanceSupplier = { mapOf("USD" to 1050.0) }
                ...
                krakenService.getBalancesCallCount = 0
                krakenService.balanceSupplier = { mapOf("USD" to 900.0) }
        ...
        "testLogOrderResult_Reflection" {
            ...
            logOrderResultMethod.invoke(
                ...
                log1, "BTC", BigDecimal.ONE, BigDecimal.TEN, "SELL"
            )
            ...
            logOrderResultMethod.invoke(
                ...
                log2, "BTC", BigDecimal.ONE, BigDecimal.TEN, "BUY"
            )
        ...
        "testBuildSnapshot_Reflection" {
            ...
            val balances = mapOf("USD" to 500.0, "BTC" to 0.01)
            val prices = mapOf("BTC" to BigDecimal("50000.0"))
            val currentValuesUSD = mapOf("USD" to BigDecimal("500.0"), "BTC" to BigDecimal("500.0"))
            ...
            val currentValuesUSDMissing = mapOf("USD" to BigDecimal("500.0"))
            ...
            val btcSnap = snapshotFallback.assets["BTC"]
