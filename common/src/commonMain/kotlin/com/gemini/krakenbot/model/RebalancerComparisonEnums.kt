package com.gemini.krakenbot.model

import com.gemini.krakenbot.view.util.ViewText

enum class ComparisonAvailability {
    AVAILABLE,
    UNAVAILABLE,
}

enum class ComparisonConfidence {
    RECONCILED,
    ESTIMATED,
}

/** Durable state of the bounded search for a later comparison start. */
enum class ComparisonProposalStatus {
    /** A candidate passed the complete reconciliation pipeline. */
    VERIFIED,

    /** The per-run budget ended while candidates remained to be checked. */
    INCOMPLETE,

    /** Every retained candidate was checked and none passed. */
    EXHAUSTED,
}

enum class ComparisonUnavailableReason(val displayText: String) {
    INSUFFICIENT_SNAPSHOTS(ViewText.UNAVAILABLE_INSUFFICIENT_SNAPSHOTS),
    NON_POSITIVE_BASELINE(ViewText.UNAVAILABLE_NON_POSITIVE_BASELINE),
    BASELINE_MISMATCH(ViewText.UNAVAILABLE_BASELINE_MISMATCH),
    MISSING_PRICE(ViewText.UNAVAILABLE_MISSING_PRICE),
    HISTORICAL_PRICE_SOURCE_ERROR(ViewText.UNAVAILABLE_HISTORICAL_PRICE_SOURCE_ERROR),
    ASSET_UNIVERSE_CHANGED(ViewText.UNAVAILABLE_ASSET_UNIVERSE_CHANGED),
    UNSUPPORTED_TRADE(ViewText.UNAVAILABLE_UNSUPPORTED_TRADE),
    AMBIGUOUS_TRADE_OWNERSHIP(ViewText.UNAVAILABLE_AMBIGUOUS_TRADE_OWNERSHIP),
    UNEXPLAINED_BALANCE_CHANGE(ViewText.UNAVAILABLE_UNEXPLAINED_BALANCE_CHANGE),
    INCEPTION_SNAPSHOT_PRUNED(ViewText.UNAVAILABLE_INCEPTION_SNAPSHOT_PRUNED),
    UNSUPPORTED_LEDGER_TYPE(ViewText.UNAVAILABLE_UNSUPPORTED_LEDGER_TYPE),
    AMBIGUOUS_LEDGER_TYPE(ViewText.UNAVAILABLE_AMBIGUOUS_LEDGER_TYPE),
    FUNDING_PROVENANCE_UNAVAILABLE(ViewText.UNAVAILABLE_FUNDING_PROVENANCE_UNAVAILABLE),
    INCEPTION_HISTORY_TRUNCATED(ViewText.UNAVAILABLE_INCEPTION_HISTORY_TRUNCATED),
    HISTORICAL_COVERAGE_GAP(ViewText.UNAVAILABLE_HISTORICAL_COVERAGE_GAP),
    INCEPTION_RECOVERY_INCOMPLETE(ViewText.UNAVAILABLE_INCEPTION_RECOVERY_INCOMPLETE),
    INCEPTION_AMBIGUOUS(ViewText.UNAVAILABLE_INCEPTION_AMBIGUOUS),
    INCEPTION_NO_BOT_EVIDENCE(ViewText.UNAVAILABLE_INCEPTION_NO_BOT_EVIDENCE),
    INCEPTION_BASELINE_UNAVAILABLE(ViewText.UNAVAILABLE_INCEPTION_BASELINE_UNAVAILABLE),
    EXTERNAL_EVIDENCE_REFRESHING(ViewText.UNAVAILABLE_EXTERNAL_EVIDENCE_REFRESHING),
    ;

    companion object {
        fun displayTextFor(reason: String?): String =
            entries.firstOrNull { it.name == reason }?.displayText ?: ViewText.UNAVAILABLE_INVALID_RESPONSE
    }
}
