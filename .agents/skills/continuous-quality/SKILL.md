...
When running under Google Antigravity (AGY), launch discovery subagents natively using built-in `invoke_subagent` tool calls; do NOT execute the Kilo-specific ARR workflow launcher. Discovery workers are read-only; test fixes, Gradle, browser tests, coverage, and final gates remain parent-owned and serial.

For optional Kilo CLI sessions, `.agents/runtime-router/adapters/kilo/route_subagents.py` with `--workflow continuous-quality` can be used.
...
1. Ensure labels exist (create once if missing):
   ...
   ```bash
   gh label create "continuous-quality" --color "1D76DB" --description "From continuous-quality QA cycles" 2>/dev/null || true
   gh label create "size/S" --color "C2E0C6" --description "Small continuous-improvement/quality item" 2>/dev/null || true
   gh label create "size/M" --color "FEF2C0" --description "Medium continuous-improvement/quality item" 2>/dev/null || true
...
2. Before creating, **dedupe**:
   ...
   # also check continuous-improvement issues for the same bug
   ```
