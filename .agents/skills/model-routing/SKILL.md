...
5. Keep direct and routed provider variants, reasoning/effort variants, and model
   ...
   verified.
6. Never put credentials, API keys, account data, raw private runtime output, or
   transient quota state in the repository or project memory.
7. Do not infer intelligence from a model name, parameter count, price, catalog
...
### 3. Add Independent Capability And Cost Evidence
...
| Availability | `verified`, `configured/unknown`, or `unavailable`, with timestamp/evidence |
...
```markdown
| Track | Primary | Effort | Cost | Capability evidence | Availability | Fallback | Substitution |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| ... | provider/model | low/medium/high | class + estimate | source + confidence | state + evidence time | provider/model | requested -> selected, reason |
```
