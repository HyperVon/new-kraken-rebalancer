...
- **New edge-case tests**: Settle failure on negative/empty balance poll (`PortfolioExecutionEdgeCasesTest`), `CancellationException` propagation in health/readiness endpoints (`DashboardOperationalApiTest`).
- **KDoc**: RFC 8259 explanation on `escapeJsonStringValue` in `ConfigServiceImpl`.

### Changed

- **Null-safety hardening**: Replaced `!!` forced unwraps in `LegacyDataRepair.kt` with `getOrNull`/`mapNotNull` and safe `CONFIRMED` fallback for corrupt legacy rows.
...
- **Skills alignment**: Unified Istanbul function threshold to 80% across 5 skills, removed volatile `/tmp` path in `kraken-api-integration`, added SSL fallback to `validate_mermaid.py`, standardized model-selection rule titles in `.cursor/` and `.clinerules/`.
