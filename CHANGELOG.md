...
- **Generic Kraken asset and ledger normalization**: Removed the hardcoded `KNOWN_LEDGER_ASSETS` whitelist and `FALLBACK_SYMBOLS` list from `Asset.kt`, and extracted Kraken exchange-specific asset codes, legacy ISO-4217 prefixes, and ticker aliases into a typed `KrakenAssetAliases` catalog in `:common` using explicit constants for all mappings. Earn/staking suffixes (`.S`, `.M`, `.F`, `.B`, `.P`) are now stripped dynamically for any arbitrary cryptocurrency symbol configured in the portfolio (e.g. `AVAX.S` → `AVAX`, `NEAR.M` → `NEAR`, `ATOM.S` → `ATOM`), ensuring reward price lookups in snapshot history work for any asset. Kraken legacy ISO-4217 asset aliases (`XXBT` → `BTC`, `ZUSD` → `USD`, `XETH` → `ETH`, `XXRP` → `XRP`, `ZEUR` → `EUR`, `ZCAD` → `CAD`, `ZGBP` → `GBP`, etc.) are mapped deterministically while preserving genuine tickers starting with X or Z (`XRP`, `XTZ`, `ZEC`, `ZETA`, `XAUT`), and USD-quoted market pair extraction in `fromTradingPair` dynamically resolves any USD-quoted trading pair even when unconfigured.
