...
- **Client-Side Kotlin/JS Cleanups (`:frontend-js`)**: Decomposed monolithic `chartDefaults` into modular, SRP Kotlin builder functions (`buildLegendConfig`, `buildTooltipConfig`, `buildScalesConfig`, `buildDefaultChartOptions`) in `History.kt`. Refactored DOM selector constants using Kotlin import aliases (`import ... as ..._QUERY`) in `Dashboard.kt` and `History.kt`, eliminating bottom-of-file boilerplate declarations. Added type-safe `DOMTokenList` extension functions (`add`, `remove`, `toggle`, `contains`) in `DomExtensions.kt` accepting `CssClass` instances directly, eliminating `.value` boilerplate across DOM manipulation code in `Dashboard.kt`, `History.kt`, `Settings.kt`, and `main.kt`.
