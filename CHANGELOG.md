...
- **Client-Side Kotlin/JS Cleanups (`:frontend-js`)**: Added type-safe `DOMTokenList` extension functions (`add`, `remove`, `toggle`, `contains`) in `DomExtensions.kt` accepting `CssClass` instances directly, eliminating `.value` boilerplate across DOM manipulation code in `Dashboard.kt`, `History.kt`, `Settings.kt`, and `main.kt`.
