...
- **Durable automatic Buy & Hold baseline verification**: when the settings comparison proves the
  ...
  path returns the proven baseline without loading snapshots, replaying trade/ledger history,
  resolving historical prices, or preparing funding evidence; reload and restart no longer
  recompute the entire Dec-5-to-present comparison merely to rediscover the same baseline.
  ...
  `comparisonStartDate` override is unaffected. 13 new tests cover persistence, same-process
  and restart fast paths, every invalidation reason, fail-closed malformed/partial state,
  tail-snapshot tolerance, and the manual override.
...
- **Inception date auto-detection & configuration**: Auto-detects strategy inception
  from the earliest multi-asset rebalance burst ($\ge 2$ distinct configured crypto symbols within
  a bounded cluster span $t - t_{\text{start}} \le 5$s) in Kraken trade history, with snapshot anchor
  proximity bounded to $\le 300$s, cached sync metadata, fallback to earliest snapshot, or an explicit
  user-configured `inceptionDate` in Settings. Resolved during startup before background retention pruning.
- **Cash-flow-adjusted All-Time High (ATH)**: External owner capital flows (`deposit`, `withdrawal`, `transfer`)
  now proportionally scale stored ATH (`Adjusted ATH = ATH * (PreFlow + NetFlow) / PreFlow`),
  preventing external deposits from resetting ATH and preventing withdrawals from triggering false drawdowns.
  Staking rewards and dividends (`REWARD_TYPES`) are treated as investment performance, improving portfolio
  value and reducing drawdown without inflating ATH.
- **Ledger watermark horizon & fail-closed pricing**: ATH ledger flow calculations are bounded by confirmed
  ledger coverage (`LEDGER_WATERMARK_EPOCH_SEC`), preventing un-synced ledger events from being skipped.
  Non-USD owner flows are priced using nearest historical portfolio snapshots ($\pm 180$s) before falling back
  to Kraken tickers; unresolved prices fail closed without advancing the watermark.
- **Inception-anchored Buy & Hold comparison**: Anchors the synthetic Buy & Hold benchmark
  ...
  preserving the cumulative delta from strategy start. Sub-window queries replay intermediate owner flows
  and manual trades into synthetic balances so sub-window queries preserve the cumulative difference in `points[0]`.
  Returns explicit `ASSET_UNIVERSE_CHANGED` on asset mismatch and trims output to inception onward when
  the window starts before inception.
...
- **Bounded historical retention policy**: Strictly protects the exact `INCEPTION_SNAPSHOT_ID` and
  owner capital ledgers (`OWNER_CAPITAL_TYPES`) on or after inception, while routine bot trades, high-frequency
  snapshots, and routine non-owner ledgers older than 90 days are pruned as scheduled.
...
- **Multi-fill order reconciliation & history sync**: Introduced `OrderFillReconciler`
  ...
  require strict volume equality at crypto precision (scale 8) or USD precision (scale 2),
  strictly rejecting partial executions (e.g. 99% or 99.9%) and overfills. Complete executions
...
- **JVM backend extraction into `:backend` subproject (ARCH01)**: Relocated JVM sources via `git mv src/ → backend/src/`, added `backend/build.gradle.kts` (ported from root, `archivesName = "kraken-bot"`, KSP argument via `rootProject.layout`, `copyJsBundle` to `backend/src/main/resources/static`), reduced root `build.gradle.kts` to an aggregator (Spotless targeting `backend/src/**/*.kt` plus Yarn resolutions, delegating `build`/`check`/`run` to `:backend`), added `include(":backend")` in `settings.gradle.kts`, updated start scripts (`.kilo/run-script`, `start.sh`/`start.bat`), CodeQL workflow manual build, and documentation (`README` subproject/package trees, `docs/EVALUATION` report paths `backend/build/reports`, `docs-screenshot-refresh`/`write-kotest`/`gradle-quality-gates` paths, `.gitignore` `backend/src/main/resources/static/rebalancer.js`), verified `./gradlew build` (88 tasks) and `:backend:fatJar` produce `kraken-bot` jars.
- **Post-extraction path remediation**: Preserved `rebalancer-config.json`/DB lookup at repo root via `JavaExec workingDir = rootProject.projectDir` and delegated `./gradlew run` → `:backend:run`, remediated stale `src/main/kotlin`/`src/test` references across `documentation-review`, `complex-code-comments`, `exposed-repository`, `ui-visual-*`/`ui-manual-qa`, `.clinerules`, `autonomous-code-optimizer/architecture-patterns`, and `skill-content-backlog` rg stragglers, corrected `README` release `build → build :backend:fatJar` and UI skill `fatJar` targets, synchronized 12 insertions across `ai-slop-detector`, `autonomous-code-optimizer`, `code-review/sample_code_review`, and `kotlin-refactoring-and-cleanup` to `backend/src/...`.
- **Skill inventories**: Updated `documentation-review` architecture and version inventories to include `:engine` and `engine/build.gradle.kts`, fixed `write-kotest` `TestFixtures` path to `backend/src/test/...`.

### Fixed

- **Documentation synchronization**: Corrected CORS override wording, bounded initial ledger-sync documentation to the 96-day seed window, refreshed evaluation scenario names, and aligned the generated JavaScript bundle path, backend coverage path, harness globs, and agent skill guidance with the current source.
- **Code hygiene (2026-08-30)**: Promoted `HtmlQueries.CSRF_TOKEN` to `const val`, simplified `LegacyDataRepair.reconcileTerminalOrderIntents` elvis return, migrated `ConfigFilePermissionStrategy` to `AclEntryPermission.entries`, refactored `OrderSettleHelper.sumMatchedSellProceeds` to `repeat(MAX_FILL_HISTORY_PAGES)` with early return and paginated `tradeId` dedupe, removed unused imports (`LedgerEvent`, `Allocation`, `TradeRecord`, `RateLimiter`, etc.), modernized `ConfigServiceTest` SAM conversion (`UserPrincipal { }`, `acl =`) and `entries` set assertions.
- **Historic changelog links**: Retargeted deep-history `CHANGELOG` `src/test/kotlin/...` links to `backend/src/test/kotlin/...` after extraction.
- **History realtime deferral**: Deferred `QO-HIST01` after reverting the SSE prototype (`#294`/`#295`) due to duplicate refreshes, accumulating sessions, and lockups; retained for a simpler bounded design with stronger lifecycle tests.
...
- **Generic Kraken asset and ledger normalization with CodeGen KrakenAssetAliases**: Removed the hardcoded `KNOWN_LEDGER_ASSETS` whitelist and `FALLBACK_SYMBOLS` list from `Asset.kt`. Kraken exchange-specific asset codes, legacy ISO-4217 prefixes, and ticker aliases are now generated at build time via `@GenerateStringConstants` from `common/src/commonMain/resources/codegen/kraken-asset-aliases.yaml` into `KrakenAssetAliases`, paired with handwritten `KrakenAssetMappings.kt` extension maps. Earn/staking suffixes (`.S`, `.M`, `.F`, `.B`, `.P`) are dynamically stripped for any cryptocurrency symbol configured in the portfolio (e.g. `AVAX.S` → `AVAX`, `NEAR.M` → `NEAR`, `ATOM.S` → `ATOM`), ensuring reward price lookups in snapshot history work for any asset. Kraken legacy ISO-4217 asset aliases (`XXBT` → `BTC`, `ZUSD` → `USD`, `XETH` → `ETH`, `XXRP` → `XRP`, `ZEUR` → `EUR`, `ZCAD` → `CAD`, `ZGBP` → `GBP`, etc.) are mapped deterministically while preserving genuine tickers starting with X or Z (`XRP`, `XTZ`, `ZEC`, `ZETA`, `XAUT`), and USD-quoted market pair extraction in `fromTradingPair` dynamically resolves any USD-quoted trading pair even when unconfigured.
- **Localize internal CodeGen compiler constants**: Removed the global `Constants.kt` grab-bag in `:codegen` and localized all private code-emitter syntax strings, regexes, and reflection type names directly into their owning processor files (`ApiMapperProcessor.kt`, `CssClassProcessor.kt`, `StringConstantProcessor.kt`, `YamlCatalog.kt`, `CatalogProcessorSupport.kt`), improving encapsulation and code locality.
