...
- **Co-locate Exposed table mappers on Table schemas**: Co-located `toModel` and `applyTo` entity mapping logic inside `LedgerTable.kt`, eliminating repetitive `LedgerTable.` prefix boilerplate in `SqliteLedgerRepositoryImpl.kt` and keeping SQL column definitions unified with model mapping logic.
