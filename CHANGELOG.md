...
- **Automatic baseline verification uses only stable, coverage-confirmed history**: the Settings
  automatic Buy & Hold baseline verification now runs against the latest stable snapshot whose
  `balancesObservedAt` is at or before `min(ledger coverage horizon, trade coverage horizon)`
  ...
  `UNEXPLAINED_BALANCE_CHANGE`. When certified coverage is unknown or fewer than two stable
