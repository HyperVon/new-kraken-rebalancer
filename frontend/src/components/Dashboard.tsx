...
const Dashboard: React.FC = () => {
    ...
    const [sortConfig, setSortConfig] = useState<{key: keyof AssetSnapshot | string, direction: 'asc' | 'desc'}>({ key: 'valueUSD', direction: 'desc' });
    ...
    const status = statusResponse?.currentStatus;
        ...
        <div className="max-w-7xl mx-auto p-4 md:p-8 space-y-8">
            ...
            <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 pb-6 border-b border-slate-800">
                <div className="flex items-center gap-4">
                    ...
                    <div className={`px-3 py-1 rounded-full text-xs font-bold tracking-wider shadow-lg ${
                ...
                <div className="flex items-center gap-6">
                    <div className="text-right">
                        ...
                        <div className={`font-mono font-bold ${isStale ? 'text-yellow-400' : 'text-slate-200'}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <StatusCard 
                    ...
                    type="neutral"
                    subValue={status?.drawdownPercent > 0 ? <span className="text-rose-400 text-sm font-medium">Drawdown: {status.drawdownPercent.toFixed(2)}%</span> : null}
                />
                <StatusCard
                    title="Cash (USD)"
                    ...
                    subValue={
                            ...
                            <div className="flex items-center gap-3 text-sm font-medium text-slate-400 mt-1">
                                ...
                                <span title="Target Allocation">
                                    ...
                                    {status.effectiveUsdTargetPercent !== undefined && Math.abs(status.effectiveUsdTargetPercent - usdAsset.targetPercent) > 0.01 &&
                                        <span className="text-xs opacity-60 ml-1">(Base: {usdAsset.targetPercent?.toFixed(2)}%)</span>
                                    }
                                </span>
                                <span className="text-slate-700">|</span>
                                <span
                        ...
                        ) : <span className="text-slate-500">No USD Data</span>
                    }
                />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="glass-panel overflow-hidden flex flex-col">
                    <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-4">Asset Performance</h2>
                    <div className="overflow-x-auto -mx-6 px-6">
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="border-b border-slate-800 text-sm text-slate-400">
                                    <th className="py-3 px-4 font-semibold cursor-pointer hover:text-white transition-colors" onClick={() => requestSort('symbol')}>Asset{getSortIndicator('symbol')}</th>
                                    <th className="py-3 px-4 font-semibold cursor-pointer hover:text-white transition-colors" onClick={() => requestSort('price')}>Price{getSortIndicator('price')}</th>
                                    <th className="py-3 px-4 font-semibold cursor-pointer hover:text-white transition-colors" onClick={() => requestSort('valueUSD')}>Value{getSortIndicator('valueUSD')}</th>
                                    <th className="py-3 px-4 font-semibold cursor-pointer hover:text-white transition-colors" onClick={() => requestSort('targetPercent')}>Target %{getSortIndicator('targetPercent')}</th>
                                    <th className="py-3 px-4 font-semibold cursor-pointer hover:text-white transition-colors" onClick={() => requestSort('currentPercent')}>Current %{getSortIndicator('currentPercent')}</th>
                                    <th className="py-3 px-4 font-semibold cursor-pointer hover:text-white transition-colors" onClick={() => requestSort('deviationPercent')}>Dev %{getSortIndicator('deviationPercent')}</th>
                                </tr>
                            </thead>
                                ...
                                {getSortedAssets().map(asset => {
                                    ...
                                    return (
                                        <tr key={asset.symbol} className="hover:bg-slate-800/30 transition-colors group">
                                            <td className="py-3 px-4 font-bold text-slate-200">{asset.symbol}</td>
                                            <td className="py-3 px-4 font-mono text-sm text-slate-300">{formatCurrency(asset.price)}</td>
                                            <td className="py-3 px-4 font-mono font-medium text-white">{formatCurrency(asset.valueUSD)}</td>
                                            <td className="py-3 px-4 text-slate-400">{asset.targetPercent?.toFixed(2)}%</td>
                                            <td className="py-3 px-4 text-slate-300">{asset.currentPercent?.toFixed(2)}%</td>
                                            <td className={`py-3 px-4 font-medium ${isBuy ? 'text-emerald-400' : (isSell ? 'text-rose-400' : 'text-slate-400')}`}>
